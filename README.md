...
> 本项目为蒟蒻计算机学生学习flutter时使用大量ai工具缝合出来的拼凑App，有大量问题且不定时维护，请谨慎使用。
>
...
- **笔记与收藏**:
    - 为重要邮件添加笔记，支持 Markdown 格式。
    - 编辑/预览双模式，实时查看格式效果。
    - 一键收藏关键邮件，方便日后查阅。
- **全文搜索**: 快速在所有邮件中查找需要的信息。
- **数据管理**:
    - 数据统计面板（邮件数量、笔记统计、趋势图表）。
    - 数据导出功能（邮件、笔记、完整数据包）。
    - 数据导入功能（支持邮件和笔记导入）。
    - 智能邮件去重（检测、分组、批量处理）。
    - 数据清理工具（归档邮件、旧邮件）。
    - 可视化图表（7天趋势、Top 10 发件人）。
- **高度可定制**:
    - 支持浅色与深色主题切换。
...
- **账户管理**: 添加、编辑或删除您的邮箱账户。
- **AI与翻译**: 配置第三方 AI 与 翻译 服务的 API Key 和 Endpoint。
- **邮件同步配置**: 设置自动同步的时间间隔。
...
<div align="center">
  <p>如果这个项目对您有帮助、使您感兴趣或者给您带来了欢乐，请给我们一个 ??</p>
